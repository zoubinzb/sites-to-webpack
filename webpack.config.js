...
module.exports = {
  ...
  plugins: [
    ...
    new CopyPlugin({
      // 应用 复制文件 插件
      patterns: [{
        from: path.resolve(__dirname, 'template/static/favicon.ico'), // 将public目录中的所有文件
    ...
    new CopyPlugin({
      // 应用 复制文件 插件
      patterns: [{
        from: path.resolve(__dirname, 'template/static/image'), // 将public目录中的所有文件
        to: './static/image', // 复制到 输出目录 的根目录
      }],
    }),
    new CopyPlugin({
      // 应用 复制文件 插件
